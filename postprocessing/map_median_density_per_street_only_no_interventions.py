...
# write_to_gpk("%s/averages/stats_by_street_full_comp.gpgk" % text, full_comp_files[0], full_comp_stats)
# calibrated compliance
# cali_comp_dict = sort_by_street(cali_comp_files)
# cali_comp_stats = calc_stats_by_street(cali_comp_dict)
# write_to_gpk("%s/averages/stats_by_street_cali_comp.gpgk" % text, cali_comp_files[0], cali_comp_stats)
...
def plot_maps(file_names):
    ...
    rows = ['{}'.format(row) for row in ['No interventions']]
    ...
    for file in file_names:
        gdf = geopandas.read_file(file)
        gdf.plot(ax=axs[0], column='mean_density', legend=True, cmap=reversed_map, vmin=0, vmax=ymax_mean)
        axs[0].legend()
        axs[0].set_xticks([])
        axs[0].set_yticks([])

        gdf.plot(ax=axs[1], column='median_density', legend=True, cmap=reversed_map, vmin=0, vmax=ymax_median)
        axs[1].legend()
        axs[1].set_xticks([])
        axs[1].set_yticks([])

        gdf.plot(ax=axs[2], column='std_density', legend=True, cmap=reversed_map, vmin=0, vmax=ymax_std)
        axs[2].legend()
        axs[2].set_xticks([])
        axs[2].set_yticks([])

        gdf.plot(ax=axs[3], column='95th_percentile', legend=True, cmap=reversed_map, vmin=0, vmax=ymax_95th)
        axs[3].legend()
        axs[3].set_xticks([])
        axs[3].set_yticks([])
    plt.show()

plot_maps(["%s/averages/stats_by_street_no_interv.gpgk" % text])
